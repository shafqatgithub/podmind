-- ============================================================
-- 38_fact_checks.sql
-- Tables for the Fact Checker (11-Feature-Specifications MODULE 8).
--
-- The module is specified — verify statistics, quotes, companies, dates;
-- return verified / partially verified / unverified with evidence and a
-- confidence score — but no tables existed for it.
--
-- A fact check belongs to a project and optionally points at the script or
-- research session it was run against, so a verdict can always be traced
...
CREATE TABLE IF NOT EXISTS public.fact_checks (
  ...
  model_name          text,
  -- Denormalised counts: a list of checks shows a verdict summary without
  -- reading every claim.
...
CREATE TABLE IF NOT EXISTS public.fact_check_claims (
  ...
  -- 0..1. Nullable because a model that cannot judge should say so rather
  -- than emit a number it does not mean.
...
-- Audit and updated_at behave as they do elsewhere.
...
-- RLS: reachable only through a project in the caller's organization.
