-- ============================================================
-- 41_discovery_and_calendar.sql
-- Topic discovery, guest discovery and the content calendar.
--
-- These complete the "topic to published episode" path the product is for:
-- discovery answers "what should this episode be about", guest suggestions
-- answer "who should be on it", and the calendar answers "when".
--
-- Discovery output is grounded in live web results rather than model recall,
-- so every topic and every suggested guest carries its sources. A trending
-- claim without a source is an opinion, and this product does not sell
-- opinions as research.
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.guest_suggestions (
    ...
    sort_order      integer NOT NULL DEFAULT 0,
    -- Set once a suggestion is promoted into a full guest briefing.
...
CREATE TABLE IF NOT EXISTS public.content_calendar (
    ...
    publish_at        date,
    guest_id          uuid REFERENCES public.guests(id) ON DELETE SET NULL,
    -- Links a planned slot to the pipeline run that produced its episode.
